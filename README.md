...
> **Disclaimer:** This is a personal research/learning project. All product data was captured from the real API using a local HTTPS proxy. No Dreamtonics servers are modified or attacked. Use responsibly and for educational purposes only.
...
- **EULA fetching** — `product/eula/get` returns real EULA markdown
...
- **UI injection** — injects a **★ Mark as Owned** button into every product card on `my.dreamtonics.com`, which creates a full permanent license and refreshes the UI live via Pinia store actions
...
| File | Description |
|---|---|
| `dreamtonics_auth.py` | Fake Keycloak OIDC server — handles login, token exchange, userinfo, account API |
| `dreamtonics_intercept.py` | authr3 API reimplementation — all product/license/client endpoints |
| `dreamtonics_inject.py` | UI injection addon — injects the ★ Mark as Owned button into the web frontend |
| `dreamtonics_logger.py` | Traffic logger — captures and pretty-prints all `authr3` + `account` traffic for further reverse engineering |
| `dreamtonics_seed.json` | Product database — 177 products, download URLs, EULAs, purchase info, i18n translations |
...
In Firefox: Settings → Network Settings → Manual proxy → HTTP `127.0.0.1` port `8080`, ✅ Also use for HTTPS.
...
Navigate to [my.dreamtonics.com](https://my.dreamtonics.com) — log in with **any email and any password**.
...
|---|---|
| ANDI Vesper Lite |
| ANDI Dawn | AUDIOLOGIE |
| ANRI Requiem SOFT | AUDIOLOGIE |
| ANRI Eclipse | AUDIOLOGIE |
| JUN Solstice | AUDIOLOGIE |
| Amara WARM | Dreamtonics Co., Ltd. |
| ZERO | AUDIOLOGIE |
