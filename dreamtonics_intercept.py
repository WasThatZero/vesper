"""
dreamtonics_intercept.py — Full authr3 API reimplementation
============================================================

Intercepts all authr3.dreamtonics.com traffic and serves responses
from a local in-memory store. Per-user state is keyed by the `sub`
claim in the Bearer JWT — shared with dreamtonics_auth.py.
...
# ── PER-USER STATE ────────────────────────────────────────────────────────────
# Keyed by JWT `sub` claim. Populated lazily on first request.
# dreamtonics_auth.py also writes to USER_STORE when a user logs in.
#
# Structure per sub:
# {
#   "email": str,
#   "given_name": str,
#   "family_name": str,
#   "licenses": { license_id: license_dict },
#   "entitlements": [],
#   "devices": [],
#   "in_beta_test": bool,
# }
...
def _sub_from_flow(flow: http.HTTPFlow) -> str:
    """Extract sub from Bearer token, falling back to dt-session cookie."""
...
class DreamtonicsIntercept:
    ...
    def _redeem_code(self, flow: http.HTTPFlow, sub: str, user: dict) -> http.Response:
        ...
        # Accept any 25-char alphanumeric code as a wildcard — grants all products
